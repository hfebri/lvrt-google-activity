...
export default function LetterDisplay({ letter, imageUrl }: LetterDisplayProps) {
  ...
  const handleDownload = () => {
    ...
    link.href = imageUrl;
    link.download = `ramadan-greeting-${Date.now()}.jpg`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  // Check if imageUrl is a short URL (Supabase) or long base64
  const isBase64 = imageUrl.startsWith('data:');
  const qrCodeUrl = isBase64
    ? (typeof window !== 'undefined' ? `${window.location.origin}/activity-1` : 'https://leverate.com')
    : imageUrl;
    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 items-start justify-center animate-in fade-in duration-700">
      ...
      <div className="flex-1 w-full max-w-md mx-auto lg:mx-0 space-y-8">
        <div className="text-center lg:text-left">
          ...
          <p className="text-slate-600">Share your heartfelt message with the world.</p>
        </div>
        ...
        <div className="glass-panel p-6 text-center">
          <p className="text-slate-600 text-sm mb-4 uppercase tracking-widest">
